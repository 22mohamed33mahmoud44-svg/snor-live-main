...
const ProfileError = ({ message, onRetry }: { message: string; onRetry: () => void }) => (
  <div dir="rtl" style={{ position:'fixed', inset:0, background:'#05050c', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', padding:24, zIndex:10000, textAlign:'center', fontFamily:"'Cairo', sans-serif" }}>
...
function App() {
  ...
  if (window.location.pathname === '/payment/success') return (
    <Suspense fallback={<Loading />}><PaymentSuccess /></Suspense>
  );
  ...
  return (
    <Suspense fallback={<Loading />}>
      <div className="min-h-screen bg-primary relative" dir={dir}>
        ...
        {user && showProfile && <Profile userId={user.id} onLogout={() => { logout(); setShowProfile(false); }} onClose={() => setShowProfile(false)} onStartRandomMatch={() => { setShowProfile(false); setShowRandomMatch(true); }} />}
        <Navbar user={user} t={t} toggleLanguage={toggleLanguage} onShowAuth={() => setShowAuth(true)} onShowProfile={() => setShowProfile(true)} />
        <Home lang={lang} t={t} user={user} onShowAuth={() => setShowAuth(true)} onStartRandomMatch={() => setShowRandomMatch(true)} />
      </div>
    </Suspense>
  );
}
