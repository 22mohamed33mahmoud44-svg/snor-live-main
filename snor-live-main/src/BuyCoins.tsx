...
export default function BuyCoins({ onClose }: { onClose?: () => void }) {
  ...
  // 1. تحميل اسكريبت الـ Widget الخاص بـ Xsolla تلقائياً عند فتح واجهة المتجر
  useEffect(() => {
  ...
  const handleBuy = async () => {
    ...
    try {
      // 2. استدعاء الدالة الجديدة المرفوعة باسمها الطويل لجلب التوكن الآمن
      ...
      // 3. تشغيل نافذة الدفع المنبثقة (Pay Station Widget)
      const XPayStationWidget = (window as any).XPayStationWidget;
      if (XPayStationWidget) {
        XPayStationWidget.init({
          access_token: data.token,
          sandbox: true // حوّلها لـ false عند الانتقال للبث الحي والفعلي (Production)
        });
        
        // نرجع حالة الشاشة للوضع الطبيعي قبل فتح النافذة
        setStep("select");
        XPayStationWidget.open();
      } else {
        // خطة بديلة: إذا تأخر تحميل السكريبت، نفتح الرابط مباشرة في نافذة جديدة
        window.open(`https://sandbox-secure.xsolla.com/paystation4/?token=${data.token}`, "_blank");
        setStep("select");
      }
