...
export default function ViewerLiveRoom({
  ...
  const initPeerConnection = useCallback(() => {
      ...
      if (event.candidate && roomChannelRef.current) {
        roomChannelRef.current.send({
          ...
          payload: {
            ...
            targetId: identityRef.current.streamerId,
            senderId: identityRef.current.myUserId,
          },
        });
      }
    };
    ...
    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected') {
        showToast('انقطع الاتصال بالبث، يتم إعادة الاتصال...');
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchOldMessages = async () => {
      ...
      if (data && isMounted) {
        ...
        safeTimeout(() => {
          if (isMounted) chatBottomRef.current?.scrollIntoView({ behavior: 'smooth' });
        }, 150);
      }
    };
    ...
    roomChannel
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'stream_chat', filter: `stream_id=eq.${streamId}` }, (payload) => {
        ...
        setMessages(prev => [...prev.slice(-(MESSAGES_LIMIT - 1)), mappedMsg]);
        safeTimeout(() => {
          if (isMounted) chatBottomRef.current?.scrollIntoView({ behavior: 'smooth' });
        }, 50);
      })
      ...
      .on('broadcast', { event: 'offer' }, async ({ payload }) => {
        if (!isMounted || payload?.targetId !== currentMyUserId) return;
        ...
        roomChannel.send({
          type: 'broadcast',
          event: 'answer',
          payload: { answer, targetId: payload.senderId, senderId: currentMyUserId },
        });
      })
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED' && isMounted) {
          roomChannel.track({ 
            user_id: currentMyUserId, 
            username: identityRef.current.myUsername, 
            role: 'viewer', 
          ...
          roomChannel.send({
            type: 'broadcast',
            event: 'viewer-join',
            payload: { viewerId: currentMyUserId },
          });
        }
      });
  ...
  const sendLike = useCallback((clientX?: number) => {
    ...
    supabase.rpc('increment_stream_likes', { target_stream_id: streamId }).then(({ error }) => {
      if (error) console.error('خطأ في مزامنة اللايك:', error.message);
    });
  }, [streamId, spawnHeart]);
  ...
  const toggleFollow = async (e: React.MouseEvent) => {
    ...
    if (isFollowing) {
      ...
      else showToast('تعذر إتمام المتابعة');
    }
  ...
  return createPortal(
    ...
    >
      <div style={{ width: '100%', maxWidth: '440px', height: '100%', background: '#000', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 0 60px rgba(0,0,0,0.8)' }}>
        ...
        <AnimatePresence>
          {sparkleBurstKey > 0 && (
            <motion.div key={sparkleBurstKey} initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0] }} exit={{ opacity: 0 }} transition={{ duration: 1.6 }} style={{ position: 'absolute', inset: 0, zIndex: 25, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '16px', boxSizing: 'border-box', pointerEvents: 'none' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'env(safe-area-inset-top)', pointerEvents: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,0.45)', padding: '4px 14px 4px 4px', borderRadius: '50px', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              {streamerId && streamerId !== myUserId && (
                <button onClick={toggleFollow} disabled={followLoading || isFollowing === null} style={{ marginLeft: 4, background: isFollowing ? 'rgba(255,255,255,0.12)' : '#ff2a74', border: isFollowing ? '1px solid rgba(255,255,255,0.25)' : 'none', color: '#fff', fontSize: '0.65rem', fontWeight: 800, padding: '4px 10px', borderRadius: '20px', cursor: followLoading ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: 4, opacity: followLoading ? 0.6 : 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%', paddingBottom: 'env(safe-area-inset-bottom)', pointerEvents: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ height: '220px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, paddingLeft: '4px' }}>
              <div style={{ flex: 1 }} />
              {messages.map((msg) => (
                <div key={msg.id} style={{ alignSelf: 'flex-start', background: 'rgba(0,0,0,0.4)', padding: '6px 14px', borderRadius: '14px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 8, maxWidth: '85%' }}>
                  {msg.user_id === myUserId && myAvatarUrl ? (<img src={myAvatarUrl} alt="" style={{ width: 18, height: 18, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />) : null}
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', width: '100%' }}>
              <form onSubmit={handleSendMessage} style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="شارك في البث..." style={{ width: '100%', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '50px', padding: '12px 40px 12px 46px', color: '#fff', fontSize: '0.85rem', outline: 'none' }} />
              ...
              <button type="button" onClick={() => setShowGiftPanel(v => !v)} style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: '#fbbf24', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Gift size={20} /></button>
              <button type="button" onClick={sendSparkle} style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(168, 85, 247, 0.15)', border: '1px solid rgba(168, 85, 247, 0.3)', color: '#c084fc', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Sparkles size={20} /></button>
              <button type="button" onClick={() => sendLike()} style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #ff2a74, #ff5388)', border: 'none', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Heart size={18} fill="currentColor" /></button>
            </div>
          </div>
        </div>
              ...
              <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 41, background: '#0f0e1c', borderTopLeftRadius: 22, borderTopRightRadius: 22, padding: '18px 16px', paddingBottom: 'calc(env(safe-area-inset-top) + 18px)', border: '1px solid rgba(255,255,255,0.08)', borderBottom: 'none' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                  {GIFTS.map(gift => {
                    ...
                    return (
                      <button key={gift.id} onClick={() => sendGift(gift)} disabled={!affordable} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '12px 6px', cursor: affordable ? 'pointer' : 'default', opacity: affordable ? 1 : 0.4 }}>
                        <span style={{ fontSize: '1.8rem' }}>{gift.emoji}</span>
                        <span style={{ color: '#fff', fontSize: '0.75rem', fontWeight: 800 }}>{gift.name}</span>
                        <span style={{ color: '#00d4ff', fontSize: '0.65rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 2 }}><Gem size={10} /> {gift.cost}</span>
                      </button>
                    );
                  })}
                </div>
