...
CREATE POLICY "waiting read own" ON waiting_users
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "waiting delete own" ON waiting_users;
CREATE POLICY "waiting delete own" ON waiting_users
...
CREATE POLICY "coin_tx read own" ON coin_transactions
  FOR SELECT USING (auth.uid() = user_id);
...
BEGIN
  -- Lock oldest waiting user (not self)
