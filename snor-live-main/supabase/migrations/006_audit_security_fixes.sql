...
-- paymob_events: RLS was enabled in 002; re-assert idempotently.
...
-- Public, safe subset for discovery / conversation lists.
-- The view runs as its owner (bypasses profiles RLS) but only
-- ever exposes non-sensitive columns.
CREATE OR REPLACE VIEW public_profiles AS
  SELECT id, username, full_name, avatar_url, gender, created_at
  FROM profiles;
